...
Since $\vec{\boldsymbol{D}}$ is normalized, we can compress it using octahedral mapping. That takes us from three 16 bit floats to 2 `N` bit Unorms. (We will decide on `N` in a moment).
